...
public class Messages {
	...
	//TODO maybe this should be in a filter, but this is small and seems reasonable to do here for now
	private Response authorizationCheck(Cookie cookie, String path, String method) {
		...
		if(cookie!=null) System.out.println("Cookie is " + cookieValue);//TODO temp for debugging
	...
	@Path("{id}/tags/{tag}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response addMessageTag(@PathParam("id") long id, @PathParam("tag") String tag, @CookieParam("categorizeus") Cookie cookie) {
	...
	@Path("{id}/tags/{tag}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response removeMessageTag(@PathParam("id") long id, @PathParam("tag") String tag, @CookieParam("categorizeus") Cookie cookie) {
